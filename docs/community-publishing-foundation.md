...
The implemented beta path covers explicit Public/Unlisted publishing, immutable sanitized WebP copies, anonymous catalog/detail/GPS/offline/report access, generated public profiles, and the exercised administrator check/changes-requested/hide/restore lifecycle. Publishing retries reuse the same request ID and unlisted secret. Exact unchanged publications stop before R2 processing. Moderation detail and asset authorization are rechecked without shared caching, corrective actions require an owner-facing reason, and application plus migration enforcement prevent owner sharing transitions from bypassing a map-level hold. Existing local and private-cloud maps are not migrated or deleted.
